...
if __name__ == "__main__":
    ...
    def transcribe_audio(file_path):
        ...
        try:
            # Google 음성 인식 API를 사용 (인터넷 연결 필요)
            text = recognizer.recognize_google(audio_data, language='ko-KR')
            return text
        except sr.UnknownValueError:
            return "음성 인식 실패"
        except sr.RequestError as e:
            return "Google 음성 인식 서비스 요청 실패; {0}".format(e)

    # 문장 단위로 음성 파일을 나누는 함수
    def split_audio_sentences(audio_path, output_folder):
        # 전체 음성 파일을 텍스트로 변환
        text = transcribe_audio(audio_path)
        sentences = text.split('. ')  # 한국어 문장 구분 기준 (점 + 공백)
        
        # 전체 음성 파일 로드
        full_audio = AudioSegment.from_file(audio_path)
        duration_per_char = len(full_audio) / len(text)  # 평균 문자당 시간
        
        start = 0
        for i, sentence in enumerate(sentences):
            end = start + len(sentence) * duration_per_char
            # 문장 별 음성 파일 분할 및 저장
            split_audio = full_audio[start:int(end)]
            split_audio.export(os.path.join(output_folder, f"sentence_{i+1}.wav"), format="wav")
            start = int(end)

    # 사용 예
    audio_path = "/yaas/storage/s1_Yeoreum/s14_VoiceStorage/81_범수(화남).wav"  # 변환할 음성 파일 경로
    output_folder = "split_audios"  # 분할된 음성 파일을 저장할 폴더
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)

    split_audio_sentences(audio_path, output_folder)