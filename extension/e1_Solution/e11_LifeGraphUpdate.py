...
def UpdateBeforeLifeGraphToSheet(BeforeLifeGraphPath, BeforeLifeGraphList):
    ...
    for i in tqdm(range(len(BeforeLifeGraphList)), desc = "[ 라이프그래프 구글시트 업데이트 ]"):
        if BeforeLifeGraphList[i]['LifeGraphFile'] is None:
            ...
            UpdateSheet(Row = row, Colum = 9, Data = Answer)
            UpdateSheet(Row = row, Colum = 13, Data = Email)
            UpdateSheet(Row = row, Type = 'Link', Colum = 15, Data = PDFPath, SubData = f'({Name})의 라이프그래프 보기/다운로드', FileName = fileName, FilePath = PDFPath)
