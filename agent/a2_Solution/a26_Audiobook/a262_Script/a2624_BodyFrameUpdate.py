...
from agent.a2_Solution.a26_Audiobook.a261_DataCommit.a2611_DataFrameCommit import FindDataframeFilePaths, AddExistedBodyFrameToDB, AddBodyFrameBodyToDB, AddBodyFrameChunkToDB, AddBodyFrameBodysToDB, BodyFrameCountLoad, SaveBodyFrame, UpdatedBodyFrame, BodyFrameCompletionUpdate
...
def LoadBodyText(projectName, email):
    ...
    if RelativeDiff >= 0.05:
        sys.exit(f"[ 분할 전후 텍스트 {RelativeDiff * 100}% 만큼 다름 (_bodyText: {Len_bodyText} != bodyText: {LenbodyText}) ]")
...
def CaptionTagging(projectName, email):
    ...
    if INPUT == OUTPUT:
        ...
        sys.exit(f"BodyText와 CaptionTaggedChunks 불일치 오류 발생: Project: {projectName} | Process: BodyFrameUpdate | CaptionTaggingMatchingError, INPUT({len(INPUT)}), OUTPUT({len(OUTPUT)})")
...
def CharacterTagging(projectName, email):
    ...
    if len(INPUT) == len(OUTPUT):
        ...
        sys.exit(f"BodyText와 CharacterTaggedChunks 불일치 오류 발생: Project: {projectName} | Process: BodyFrameUpdate | CharacterTaggingMatchingError, INPUT({len(INPUT)}), OUTPUT({len(OUTPUT)})")
...
def IndexMatching(projectName, email):
    ...
    if INPUT != OUTPUT:
        sys.exit(f"CharacterTaggedChunks와 IndexMatchedChunks 불일치 오류 발생: Project: {projectName} | Process: BodyFrameUpdate | IndexMatchedChunksError, INPUT({len(INPUT)}), OUTPUT({len(OUTPUT)})")
    else:
...
def TaggedChunksToUnitedChunks(projectName, email, tokensCount):
    ...
    if INPUT != OUTPUT:
        sys.exit(f"IndexMatchedChunks와 IndexMatchedChunks 불일치 오류 발생: Project: {projectName} | Process: BodyFrameUpdate | TaggedChunksToUnitedChunksError, INPUT({len(INPUT)}), OUTPUT({len(OUTPUT)})")
    else:
...
def SplitedBodyScriptsToBodys(projectName, email):
    ...
    if INPUT != OUTPUT:
        sys.exit(f"SplitedBodyScripts와 Bodys 불일치 오류 발생: Project: {projectName} | Process: BodyFrameBodysUpdate | BodyFrameBodysUpdateError, INPUT({len(INPUT)}), OUTPUT({len(OUTPUT)})")
    else:
...
def BodyFrameBodysUpdate(projectName, email):
    ...
    for Update in UpdateTQDM:
        ...
        UpdateTQDM.set_description(f'BodyFrameBodysUpdate: {BodyPreview} ...')
        time.sleep(0.0001)
        ...
        if ExistedDataFrame != None:
            ...
            for Update in UpdateTQDM:
                ...
                for j in range(len(IndexBodyUnitChunksList[i])):
                    DataFrame = AddBodyFrameBodyToDB(DataFrame, IndexId, IndexTag, IndexChunk)
                    ...
                    for k in range(len(IndexBodyUnitChunksList[i][j])):
                        ...
                        DataFrame = AddBodyFrameChunkToDB(DataFrame, ChunkId, Tag, Chunk)
                # i값 수동 업데이트
                i += 1
            ...
            if INPUT != OUTPUT:
                sys.exit(f"IndexBodyUnitChunksList와 BodyFrameList 불일치 오류 발생: Project: {projectName} | Process: BodyFrameUpdate | BodyFrameUpdateError, INPUT({len(INPUT)}), OUTPUT({len(OUTPUT)})")
            else:
