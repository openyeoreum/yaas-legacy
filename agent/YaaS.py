...
## YaaSConfig 로드
def LoadYaaSConfig(UserEmail, ProjectName):
    ConfigPath = f'/yaas/storage/s1_Yeoreum/s12_UserStorage/{UserEmail}_user/{UserEmail}_storage/{ProjectName}/{ProjectName}_config.json'
...
## YaaSConfig 설정
def YaasConfigUpdate(UserEmail, ProjectName, MainLang, Estimate, DataCollection, Search, Script, Translation, TextBook, AudioBook, Marketing):

    ## YaaSConfig 로드
    YaasConfig, ConfigPath = LoadYaaSConfig(UserEmail, ProjectName)
...
def YaaSMultiProcessing(User, ProjectName, MainLang, Estimate, DataCollection, Search, Script, Translation, TextBook, AudioBook, Marketing, MessagesReview, Account):
    ...
    if os.path.exists(f'/yaas/storage/s1_Yeoreum/s12_UserStorage/{UserEmail}_user/{UserEmail}_storage/'):
        for storageProjectName in os.listdir(f'/yaas/storage/s1_Yeoreum/s12_UserStorage/{UserEmail}_user/{UserEmail}_storage/'):
            if os.path.isdir(f'/yaas/storage/s1_Yeoreum/s12_UserStorage/{UserEmail}_user/{UserEmail}_storage/{storageProjectName}'):
    ...
    if ProjectName not in storageProjectNameList:
        YaasConfigUpdate(UserEmail, ProjectName, MainLang, Estimate, DataCollection, Search, Script, Translation, TextBook, AudioBook, Marketing)
    ## Config 불러오기
    YaasConfig, ConfigPath = LoadYaaSConfig(UserEmail, ProjectName)
...
if __name__ == "__main__":
    ...
    AudioBook = "한예림(낭독)" # '', 'Auto', '클로닝성우이름', '성우이름(특성)' ... 중 선택
