...
from agent.a2_Solution.a26_Audiobook.a261_DataCommit.a2611_DataFrameCommit import FindDataframeFilePaths, LoadOutputMemory, SaveOutputMemory, AddExistedDuplicationPreprocessToDB, AddDuplicationPreprocessScriptsToDB, DuplicationPreprocessCountLoad, UpdatedDuplicationPreprocess, DuplicationPreprocessCompletionUpdate
...
def PreprocessAndSplitScripts(bodyText, indexFrame, mainLang):
    ...
    for ScriptInQuotes in doubleQuotesScripts:
        ScriptInQuotesTokens = re.sub(r'\s+', ' ', ScriptInQuotes).strip().split()
        if len(ScriptInQuotesTokens) > 100:
            sys.exit(f"[ 쌍따옴표 내부의 문장이 너무 길어(단어수, {len(ScriptInQuotesTokens)}) 확인 필요: {' '.join(ScriptInQuotesTokens[:6])} ... {' '.join(ScriptInQuotesTokens[6:])}]\n")
...
def DuplicationPreprocessProcess(projectName, email, mainLang, DataFramePath, Process = "DuplicationPreprocess",  memoryLength = 2, MessagesReview = "on", Mode = "Memory", MainLang = "ko"):
