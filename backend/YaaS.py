...
if __name__ == "__main__":
    ...
    ProjectNameList = {"StartProjectName": "", "ContinueProjectNameList": ["250304_데미안"]}
    ...
    # # 환경변수에서 API 키 가져오기 (없으면 직접 문자열로 지정)
    ...
    #     "language": "ko",
    #     "page": 0,
    #     "page_size": 1000  # 최대 페이지 크기로 설정 (API 최대값이 100이라 가정)
    # }
    ...
    # # 가져온 전체 데이터를 JSON 파일로 저장
    # output_file = "/yaas/supertone.json"
    # with open(output_file, "w", encoding="utf-8") as f:
    #     json.dump(all_voices, f, indent=2, ensure_ascii=False)

    # print(f"전체 음성 데이터를 {output_file} 파일로 저장했습니다. (총 {len(all_voices)}개)")
    ...
    headers = {"x-sup-api-key": SUPERTONE_API_KEY}  # API 키를 입력하세요

    response = requests.get(url, headers=headers)

    if response.status_code == 200:
        data = response.json()
        output_file = "supertone.json"
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        print(f"전체 리스트가 {output_file} 파일로 저장되었습니다.")
    else:
        print("오류 발생:", response.status_code, response.text)