...
def EditToScriptIntroductionGenInputList(ScriptEditPath, BeforeProcess1, BeforeProcess2):
    with open(ScriptEditPath, "r", encoding = "utf-8") as ScriptEditJson:
...
def ScriptPlanProcessDataFrameSave(ProjectName, BookScriptGenDataFramePath, ProjectDataFrameScriptPalnPath, ScriptPlanResponse, Process, InputCount, TotalInputCount):
    ...
    if os.path.exists(ProjectDataFrameScriptPalnPath):
        ...
        ScriptPlanFramePath = os.path.join(BookScriptGenDataFramePath, "a5312-01_ScriptPlanFrame.json") 
...
def TitleAndIndexGenProcessDataFrameSave(ProjectName, BookScriptGenDataFramePath, ProjectDataFrameTitleAndIndexPath, TitleAndIndexGenResponse, Process, InputCount, TotalInputCount):
    ...
    if os.path.exists(ProjectDataFrameTitleAndIndexPath):
        ...
        TitleAndIndexFramePath = os.path.join(BookScriptGenDataFramePath, "a5312-02_TitleAndIndexFrame.json")
...
def SummaryOfIndexGenProcessDataFrameSave(ProjectName, BookScriptGenDataFramePath, ProjectDataFrameSummaryOfIndexPath, SummaryOfIndexGenResponse, Process, InputCount, TotalInputCount):
    ...
    if os.path.exists(ProjectDataFrameSummaryOfIndexPath):
        ...
        SummaryOfIndexFramePath = os.path.join(BookScriptGenDataFramePath, "a5312-03_SummaryOfIndexFrame.json")
...
## Process9: ScriptIntroductionGenProcess DataFrame 저장
def ScriptIntroductionGenProcessDataFrameSave(ProjectName, BookScriptGenDataFramePath, ProjectDataFrameScriptIntroductionGenPath, ScriptIntroductionGenResponse, Process, InputCount, TotalInputCount):
    ## ScriptIntroductionGenFrame 불러오기
    if os.path.exists(ProjectDataFrameScriptIntroductionGenPath):
        ...
        ScriptIntroductionFramePath = os.path.join(BookScriptGenDataFramePath, "a5312-04_ScriptIntroductionFrame.json")
    with open(ScriptIntroductionFramePath, 'r', encoding = 'utf-8') as DataFrameJson:
        ScriptIntroductionFrame = json.load(DataFrameJson)
        
    ## ScriptIntroductionFrame 업데이트
    ScriptIntroductionFrame[0]['ProjectName'] = ProjectName
    ...
    if InputCount == TotalInputCount:
        ScriptIntroductionFrame[0]['Completion'] = 'Yes'
...
def ShortScriptGenProcessDataFrameSave(ProjectName, BookScriptGenDataFramePath, ProjectDataFrameShortScriptPath, ShortScriptGenResponse, Process, InputCount, TotalInputCount):
    ...
    if os.path.exists(ProjectDataFrameShortScriptPath):
        ...
        ShortScriptFramePath = os.path.join(BookScriptGenDataFramePath, "a5312-05_ShortScriptFrame.json")
...
def BookScriptGenProcessUpdate(projectName, email, Intention, mode = "Master", MessagesReview = "on"):
    ...
    ProjectScriptPath = f"/yaas/storage/s1_Yeoreum/s12_UserStorage/{email}_user/{email}_storage/{projectName}/{projectName}_script"
    ...
    BookScriptGenDataFramePath = "/yaas/agent/a5_Database/a53_ProjectData/a531_ScriptProject/a5312_BookScriptGen"
    ...
    ## Process 설정
    ProcessNumber = '04'
    Process = "ScriptIntroductionGen"
    
    ## ScriptIntroductionGen 경로 생성
    ProjectDataFrameScriptIntroductionGenPath = os.path.join(ProjectDataFrameScriptPath, f'{email}_{projectName}_{ProcessNumber}_{Process}DataFrame.json')
    ...
    if not EditCheck:
        if DataFrameCompletion == 'No':
            for i in range(InputCount - 1, TotalInputCount):
                ...
                ## Response 생성
                ScriptIntroductionGenResponse = ProcessResponse(projectName, email, Process, Input, inputCount, TotalInputCount, ScriptIntroductionGenFilter, CheckCount, "OpenAI", mode, MessagesReview)
                
                ## DataFrame 저장
                ScriptIntroductionGenProcessDataFrameSave(projectName, BookScriptGenDataFramePath, ProjectDataFrameScriptIntroductionGenPath, ScriptIntroductionGenResponse, Process, inputCount, TotalInputCount)
                
        ## Edit 저장
        ProcessEditSave(ProjectDataFrameScriptIntroductionGenPath, ScriptEditPath, Process)
        sys.exit(f"[ {projectName}_Script_Edit 생성 완료 -> {Process}: (({Process}))을 검수한 뒤 직접 수정, 또는 수정 사항을 ((<prompt: >))에 작성, 수정사항이 없을 시 (({Process}Completion: Completion))으로 변경 ]\n{ScriptEditPath}")
