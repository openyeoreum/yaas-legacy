...
if __name__ == "__main__":
    ...
    # 1. 데이터 준비 (예: 데이터를 읽어와서 보고서로 변환)
    def prepare_report():
        # 여기서는 예시로 간단한 데이터프레임을 생성합니다.
        data = {
            'Date': [datetime.date.today() - datetime.timedelta(days=i) for i in range(5)],
            'Sales': [150, 200, 170, 250, 300],
        }
    ...
    def create_email_body(report):
        ...
        <p>Below is the sales report for today:</p>
        {report}
        """
        return body
    ...
    def send_email(body):
        # 이메일 설정
        yag = yagmail.SMTP('yeoreum00128@gmail.com', 'Dhvmsdufma1!')
        
        # 수신자 이메일 주소와 제목
        to = 'junsun0128@gmail.com'
        subject = f"Daily Sales Report - {datetime.date.today().strftime('%Y-%m-%d')}"
